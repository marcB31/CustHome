			...
			<table>
				<thead>
					<tr>
						...
						<g:sortableColumn property="cars_id" title="${message(code: 'reservation.cars_id.label', default: 'Carsid')}" />
					
						<g:sortableColumn property="customer_id" title="${message(code: 'reservation.customer_id.label', default: 'Customerid')}" />
				...
				<g:each in="${reservationInstanceList}" status="i" var="reservationInstance">
					<tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
						...
						<td><g:link action="show" id="${reservationInstance.id}">${fieldValue(bean: reservationInstance, field: "cars_id")}</g:link></td>
					
						<td>${fieldValue(bean: reservationInstance, field: "customer_id")}</td>
